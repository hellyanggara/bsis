...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed text-sm">
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-6">
                            <form id="tambahMapping" method="post">  
                                <div class="card card-primary">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label>Pegawai</label>
                                            <select name="id_pegawai" id="id_pegawai" class="form-control select2 wajib">
                                                <option></option>
                                                <?php
                                                while($dtPegawai = $qPegawai->fetch_object()){
                                                echo '<option value="'.$dtPegawai->idpegawai.'">'.$dtPegawai->namapegawai.'</option>';
                                                }
                                                ?>
                                            </select>
                                        ...
                                        <div class="form-group">
                                            <label><?php echo $labelAdd?></label>
                                            <select name="id_primary" id="id_primary" class="form-control select2 wajib">
                                                <option></option>
                                                <?php
                                                while($dtAdditional = $qAdditional->fetch_object()){
                                                echo '<option value="'.$dtAdditional->id.'">'.$dtAdditional->nama.'</option>';
                                                }
                                                ?>
                                            </select>
                                            <span class="error invalid-feedback"><?php echo $labelAdd?> harus dipilih</span>
                                        </div>
                                    </div>
                                    <div class="card-footer">
                                        <button class="btn btn-primary btn-sm float-right" name="simpan">Simpan</button>
                                        <a href="<?= $httpHost ?>mapping/user/?ref=<?php echo $slug ?>" class="btn btn-danger btn-sm">Batal</a>
                                    </div>
                                </div>
                            </form>
                        </div>
    ...
    <script>
        ...
        $("#tambahMapping").submit(function(e) {
            e.preventDefault();
            var all = $(".wajib");
            all.each(function() {
                if($(this).val() == ''){
            ...
            if(valid){
                ...
                confirmButtonText: 'Ya, simpan!'
                    ...
                    if (result.isConfirmed) {
                        data = $("#tambahMapping").serialize() + "&slug=<?php echo $slug ?>";
                        // console.log(data)
                        $.ajax({
                            type: "POST",
                            ...
                            success: function(response) {
                                ...
                                if(response.status == 'sukses'){
                                    window.location = "<?php echo $httpHost ?>mapping/user/?ref=<?php echo $slug ?>"
                                }else{
                                    window.location = ""
                                }
                            }
